...
title Atualizacao do Conector Razync 2.4
color 0B
...
set "RAZYNC_TEMP=%TEMP%\RazyncConnector240"
set "RAZYNC_LOG=%TEMP%\RazyncConnector240.log"

echo.
echo  ==========================================
echo       CONECTOR RAZYNC 2.4 - PROCURADOR PJ
echo  ==========================================
echo.
echo  Corrige o preenchimento do CNPJ da empresa representada.
...
echo  Baixando os componentes 2.4...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "try { $ErrorActionPreference='Stop'; $base='https://raw.githubusercontent.com/rodrigofrrsilva1703-a11y/Razync/main/connector_windows/'; $files=@('connector.py','configure_chrome.ps1','automate_ecac.ps1'); foreach($f in $files){Invoke-WebRequest -UseBasicParsing -Headers @{'Cache-Control'='no-cache';'Pragma'='no-cache'} -Uri ($base+$f+'?build=240') -OutFile (Join-Path $env:RAZYNC_TEMP $f)}; exit 0 } catch { Write-Error $_.Exception.Message; exit 1 }" > "%RAZYNC_LOG%" 2>&1
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "try { $r=Invoke-RestMethod -UseBasicParsing -Uri 'http://127.0.0.1:17891/v1/health' -TimeoutSec 8; if(-not $r.ok -or $r.version -ne '0.6.2'){throw ('Versao inesperada: '+$r.version)}; Write-Output ('Conector ativo - versao '+$r.version); exit 0 } catch { Write-Error $_.Exception.Message; exit 1 }" >> "%RAZYNC_LOG%" 2>&1
if errorlevel 1 goto erro

rmdir /s /q "%RAZYNC_TEMP%" 2>nul
color 0A
echo.
echo  INSTALACAO CONCLUIDA - CONECTOR 2.4
